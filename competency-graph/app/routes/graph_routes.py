...
from logic.graph_utils import parse_all_skills_recursive, get_all_skills, SKILLS_URL
...
def get_graph_data():
    ...
    # Dynamic sätted enne parse käivitust
    ...
    try:
        ...
        data, depths = parse_all_skills_recursive(skill_list)

        if not data or all(
                len(info["subskills"]) == 0 and
                len(info["prerequisites"]) == 0 and
                len(info["competency"]) == 0
